...
def tracker(request):
    ...
    return render(request,'ecomm/tracker.html')
def search(request):
    return render(request,'ecomm/search.html')
def prodview(request,myid):
    product = Product.objects.filter(id=myid)
    return render(request,'ecomm/prodview.html',{'product':product[0]})
def checkout(request):
    if request.method == "POST":
        ...
        thank = True
        id = order.order_id
        return render(request, 'ecomm/checkout.html', {'thank': thank, 'id': id})
